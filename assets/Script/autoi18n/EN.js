if (!window.autoi18n) window.autoi18n = {};
window.autoi18n['EN'] = {
    "showText": {
        "cjfjtips": "请输入创建房间信息",
        "rssrtips": "人数输入有误",
        "jrsrtips": "金额输入有误，最小不小于10",
        "sjsrtips": "时间输入有误",
        "czerrortips": "操作错误",
        "ssfjbcztips": "搜索的房间不存在",
        "myhytips": "您还没有好友哦",
        "hqhylbtips": "获取列表失败",
        "fsyqxxsustips": "发送邀请信息成功",
        "xzyqhytips": "请选择需要邀请的好友",
        "wldktips": "网络断开，正在重连...",
        "fhbnwktips": "房号不能为空",
        "ssmmjrtips": "请输入密码后进入",
        "zhwjtc": "最后一位玩家退出，房间即将解散。",
        "qdtcyx": "您确定退出游戏吗？",
        "xtydj": "系统已冻结您",
    },
    "serverTips":{
        "Lang10001": "您已进入{0}号房间,无法创建房间",
        "Lang10002": "您已进入{0}号房间",
        "Lang10003": "房间密码错误~",
        "Lang10004": "房间人数已满~",
        "Lang10005": "房间人数已满或已开始游戏~",
        "Lang10006": "用户信息获取失败,请重新登录~",
        "Lang10007": "游戏已开始",
        "Lang10008": "请先进入房间",
        "Lang10009": "已准备,请耐心等待开始游戏~",
        "Lang100011": "用户信息获取失败,请重新登录~",
        "Lang100012": "ALSC币余额不足~",
        "Lang100013": "游戏中不允许退出",
        "Lang100014": "游戏已结束",
        "Lang100015": "该矿石已被其他玩家挖出",
        "Lang100016": "该矿石已被其他玩家抢先一步",
        "Lang100017": "亲,房主才能解散房间哦~",
        "Lang100018": "游戏已开始,无法解散房间~",
        "Lang100019": "没有此房间~",
        "Lang100020": "房间不存在,请重试~",
        "Lang100021": "连接错误",
        "Lang100022": "链接信息错误",
    },

    "inviteText": {
        "sdyqxx": "收到邀请信息",
    },

    "HallScene": {
        "hallUI.text_RoomList": "房间列表",
        "RoomListIcon.headicondi.l_fangzhu": "房主",
        "RoomListIcon.i_danzhu": "单注：",
        "RoomListIcon.i_wanjia": "玩家：",
        "RoomListIcon.i_fanghao": "房号：",
    },

    "GameScene": {
        "upNode.leftdi.nicheng": "昵称：",
        "upNode.leftdi.defen": "得分：",
        "upNode.rightdi.fanghao": "房号：",
        "bjyxhd": "本局游戏您获得",
    },

    "CreateRoom": {
        "label_password": "游戏人数",
        "label_renshu": "游戏时间",
        "label_time": "单注金额",
        "label_jine": "房间密码",
        "label_tips": "创建房间需要冻结",
        "EditBoxRenshu.label_ren": "人",
        "EditBoxTime.label_fen": "分",
        "EditBoxRenshu.PLACEHOLDER_LABEL": "最多20人",
        "EditBoxTime.PLACEHOLDER_LABEL": "请输入游戏时长",
        "EditBoxJine.PLACEHOLDER_LABEL": "请输入整数金额",
        "EditBoxMima.PLACEHOLDER_LABEL": "可不填",
    },

     "GameRule": {
        "ruleScrollView.view.content.ruleLabel": `经典游戏“黄金矿工”的高清版本，画面更加精致，游戏内容更加丰富。
        保留了经典的游戏玩法，增加了养成系统，让游戏更加有趣。同时支持三个人一起玩，快点叫上你的朋友一起来吧！经典游戏“黄金矿工”的高清版本，画面更加精致，
        游戏内容更加丰富。保留了经典的游戏玩法，增加了养成系统，让游戏更加有趣。同时支持三个人一起玩，快点叫上你的朋友一起来吧！`,
     },

     "GetInvite": {
        "tip": "邀请你加入游戏"
     },

      "SearchRoom": {
        "fanghaoEditBox.PLACEHOLDER_LABEL": "请输入房间号",
        "mimaEditBox.PLACEHOLDER_LABEL": "请输入房间密码",
     },
}